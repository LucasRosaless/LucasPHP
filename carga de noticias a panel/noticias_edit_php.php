<?php
	...
	$tipo = $_POST['tipo'];
	$sql = "UPDATE ".$tabla." SET titulo='".addslashes($titulo)."', encabezado='".addslashes($encabezado)."', descripcion='".addslashes($descripcion)."', tipo='$tipo', categoria='$categoria', imagen='$fisico', video='$video', fuente='$fuente', mostrar='$mostrar', fecha='$fecha', audio='$audio' WHERE id ='$id'";

	if(!$resultado = $mysqli->query($sql)){
		
		$mensaje_error = 'Algo salio mal, no se pudo actualizar la noticia';
	}else{
		$mensaje_error = 'Se pudo actualizar la noticia';
		...
		for($i = 0; $i < $num; $i++) {
			...
			$sql = "INSERT INTO imagen_noticias (imagen, id_noticia, mostrar) VALUES('".$_FILES['img']['name'][$i]."', '$id', '1')";
			if(!$resultado = $mysqli->query($sql)){
